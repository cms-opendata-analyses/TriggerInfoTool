...
{
	C = fs->make <TCanvas>();
  h1 = fs->make <TH1D>("h1","obj pt with full circles",100,0,300);
  h2 = fs->make <TH1D>("h2","obj pt with full squares",100,0,300);
  h3 = fs->make <TH1D>("h3","obj pt with full triangles up",100,0,300);
  h4 = fs->make <TH1D>("h4","obj pt with full triangles down",100,0,300);
...
void TriggerPrescalesAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  const vector<string> triggerNames1 = hltConfig_.datasetContent("HLT_Jet60");

	for (unsigned i = 0; i < triggerNames1.size(); i++) {

	string name = triggerNames1[i];
	if (!checkTriggerPass(iEvent, name)){
 	...
 	trigger::size_type filterIndex = triggerEventHandle_->filterIndex(edm::InputTag(filterName1_,"",triggerEventTag_.process()));
  ...
  const vector<string> triggerNames2 = hltConfig_.datasetContent("HLT_Jet100");

	for (unsigned i = 0; i < triggerNames2.size(); i++) {

	string name = triggerNames2[i];
	if (!checkTriggerPass(iEvent, name)){
				return;
			}
 	...
 	trigger::size_type filterIndex = triggerEventHandle_->filterIndex(edm::InputTag(filterName2_,"",triggerEventTag_.process()));
  ...
  const vector<string> triggerNames3 = hltConfig_.datasetContent("HLT_Jet140");

	for (unsigned i = 0; i < triggerNames3.size(); i++) {

	string name = triggerNames3[i];
	if (!checkTriggerPass(iEvent, name)){
				return;
			}
 	...
 	trigger::size_type filterIndex = triggerEventHandle_->filterIndex(edm::InputTag(filterName3_,"",triggerEventTag_.process()));
  ...
  const vector<string> triggerNames4 = hltConfig_.datasetContent("HLT_Jet200");

	for (unsigned i = 0; i < triggerNames4.size(); i++) {

	string name = triggerNames4[i];
	if (!checkTriggerPass(iEvent, name)){
				return;
			}
 	...
 	trigger::size_type filterIndex = triggerEventHandle_->filterIndex(edm::InputTag(filterName4_,"",triggerEventTag_.process()));
